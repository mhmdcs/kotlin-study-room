...
//In the class A example, the parameter must be a property (var or val) because it is used inside a function:
...
//In this class B example, the parameter is only used to initialize the class, so it does not need to be a property.
//it is simply a parameter passed to the primary constructor, where it can be used it to initialize other properties, or accessed within an init block.
class B(number: Int) {
    val value = number
    init {
